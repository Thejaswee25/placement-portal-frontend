      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-16 text-neutral-400 text-xs">Profile not found.</div>
          ) : (
                ...
                {profile.resumeUrl ? (
                  <button onClick={handleDownload} disabled={dl}
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-neutral-800 truncate">{profile.name}_resume.pdf</p>
                      <p className="text-[11px] text-neutral-500">Download PDF</p>
                    </div>
                    {dl
                  ...
                  <p className="text-xs text-neutral-400 py-3 text-center rounded-lg border border-dashed border-neutral-200">
                    No resume uploaded
                  </p>
                )}
              </div>
...
export default function ReviewApplications() {
  ...
  const fetchApps = () =>
    getAllApplicationsApi().then(({ data }) => setApps(data)).catch(console.error).finally(() => setLoading(false))
  ...
  return (
    <DashboardLayout title="Applications">
      ...
      {selected && (
        <StudentPanel app={selected} onClose={() => setSelected(null)}
                      onAction={doAction} acting={acting} />
      )}
    </DashboardLayout>
  )
}
