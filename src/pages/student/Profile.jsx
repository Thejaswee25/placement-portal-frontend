...
export default function Profile() {
  ...
  const [form, setForm] = useState({
    branch: '', cgpa: '', graduationYear: '', phone: '',
    ...
    getProfileApi()
      .then(({ data }) => {
        ...
        setForm({
          ...
          graduationYear: data.graduationYear ?? '', phone: data.phone ?? '',
          linkedin: data.linkedin ?? '', github: data.github ?? '', skills: data.skills ?? [],
        })
      })
      .catch(() => {})
  ...
  const handleSave = async (e) => {
    ...
    try {
      const { data } = await updateProfileApi({
        ...form, cgpa: parseFloat(form.cgpa) || null, graduationYear: parseInt(form.graduationYear) || null,
      })
      setProfile(data); showMsg('success', 'Profile saved.')
  ...
  const handleUpload = async (e) => {
    ...
    if (file.type !== 'application/pdf') { showMsg('error', 'Only PDF files are allowed.'); return }
    setUploading(true)
  ...
  const Field = ({ label, name, type = 'text', placeholder, prefix }) => (
    <div>
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="card mb-4 flex items-center justify-between gap-4">
          ...
          <div>
            ...
            <button onClick={() => fileRef.current.click()} disabled={uploading} className="btn-secondary text-xs py-1.5">
              <Upload size={12} />{uploading ? 'Uploading…' : profile?.resumeUrl ? 'Replace Resume' : 'Upload Resume'}
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="card space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="CGPA"            name="cgpa"           type="number" placeholder="8.5" />
            <Field label="Graduation Year" name="graduationYear" type="number" placeholder="2025" />
          ...
          <div className="flex gap-2">
            <input value={skillInput} onChange={e => setSkillInput(e.target.value)}
                   ...
                   placeholder="Add a skill and press Enter" className="input flex-1 text-xs" />
            <button type="button" onClick={addSkill} className="btn-secondary px-2.5 py-2"><Plus size={14} /></button>
          </div>
          {form.skills.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {form.skills.map(s => (
                <span key={s} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs
                                         font-medium bg-primary-50 text-primary-700 ring-1 ring-primary-200/60">
                  {s}
          ...
          <div className="flex justify-end pt-1">
            <button type="submit" disabled={saving} className="btn-primary text-xs">
              {saving ? <span className="flex items-center gap-1.5"><span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving…</span> : 'Save Profile'}
            </button>
          </div>
        </form>
      </div>
